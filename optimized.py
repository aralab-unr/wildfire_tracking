...
def RandomJoinAction(V, number_actions, F, X_MIN, X_MAX, Y_MIN, Y_MAX, Z_MIN, Z_MAX):
    ...
    print(current_state_agent_1)
    print(current_state_agent_2)
    print(current_state_agent_3)
...
def EstimateGlobalReward(F, V, FOV_angle, X_MIN, X_MAX, Y_MIN, Y_MAX):
    ...
    if (np.sum(reward_array) + 0.1 >= total_point) and (np.sum(overlap_point_array) <= 1): # overlapping == 1 still meaning ==0, because need at least 2
        ...
        terminal_state = True
        print ("Rewards: ", CRi, np.sum(reward_array), total_point, np.sum(overlap_point_array))
    else:
...
F[3, 2] = high_intensity
F[4, 2] = high_intensity
F[2, 3] = high_intensity
F[5, 3] = high_intensity
F[3, 4] = high_intensity
F[6, 4] = high_intensity
F[3, 5] = high_intensity
F[5, 5] = high_intensity
F[4, 6] = high_intensity
